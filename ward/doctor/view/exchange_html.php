	...
	<div class="columns">
		...
		<div class="column is-8">
			<div class="box" data-aos="flip-right" data-aos-duration="800" data-aos-once="true">
				<h2 class="has-text-centered subtitle"><i class="fas fa-exchange-alt"></i>&thinsp;病人换房</h2>
					<?php
						if(empty($exchange_list)):
					?>
						<p class="has-text-centered">暂无病人提交申请</p>
					<?php
						else:
					?>
					...
					?>
								...
								<tr>
									<td colspan="6">
										目标病房:&nbsp;<?=$row['building']?>号楼 <?=$row['number']?>户（<?=$row['bed']?>人间，已住<?=$row['count']?>人）
									</td>
								</tr>
								<tr>
									<?php
									...
									?>
										...
										<td colspan="2">
											<div class="buttons">
												<a class="button is-small is-outlined is-success" onclick="checkYes(<?=$row['id']?>,'<?=$row['patient_name']?>')">批准</a>
												<a class="button is-small is-outlined is-dark" onclick="checkNo(<?=$row['id']?>,'<?=$row['patient_name']?>')">否决</a>
											</div>
										</td>
									<?php
										else:
					...
					?>
						...
						<script>
							function checkYes(id,name){
								var r = confirm("确定批准"+name+"患者的换宿申请吗？");
								if (r == true) {
									$("#id").val(id);
									$("#response").val("同意");
								    $("form").submit();
							...
							function checkNo(id,name){
								var r = confirm("确定否决"+name+"患者的换宿申请吗？");
								if (r == true) {
									$("#id").val(id);
									$("#response").val("不同意");
								    $("form").submit();
