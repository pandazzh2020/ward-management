		...
		<div class="column is-8">
			<div class="box" data-aos="flip-right" data-aos-duration="800" data-aos-once="true">
				<div class="has-text-centered">
					...
					?>
							...
							&thinsp;<i class="fas fa-chevron-right"></i>&nbsp;搜索病人
					<?php
						else:
					?>
							<h2 class="has-text-centered subtitle"><i class="fas fa-user-graduate"></i>&thinsp;搜索病人</h2>
					<?php
						endif;
					?>
				</div>
					...
					<form action="./patient.php" method="GET">
						<div class="columns">
							<div class="column is-9">
								<input class="input" type="text" name="keyword" placeholder="账号或姓名" required="required" maxlength="20" value="<?=isset($keyword)?$keyword:''?>"/>
							</div>
				...
				<br>
					<?php
						if(empty($patient_list)):
							if(isset($keyword)):
					?>
								<p class="has-text-centered"><i class="fas fa-question"></i>&thinsp;查无此病人</p>
					<?php
							else:
					?>
					...
					?>
								<tr>
									...
									<td>
										...
										?>
											<a href="./ward_detail?id=<?=$row['ward_id']?>">
												<?=$row['building']?>号楼&nbsp;<?=$row['number']?>室
											</a>
										<?php
											endif;
										?>
			...
			<div class="box" data-aos="flip-right" data-aos-duration="800" data-aos-once="true">
				<div class="has-text-centered">
					...
					?>
							<a class="subtitle"><?=$html_func?></a>
							&thinsp;<i class="fas fa-chevron-right"></i>&nbsp;全部病人
					<?php
						else:
					?>
							<h2 class="has-text-centered subtitle"><i class="fas fa-user-graduate"></i>&thinsp;全部病人</h2>
					<?php
						endif;
					?>
				</div>
				<br>
					...
					<?php
							foreach($all_patient_list as $row):
					?>
								<tr>
									<td>
										<?=$row['account']?>
					...
					?>
							...
							<br>
							<nav class="pagination is-centered" role="navigation" aria-label="pagination">
							  <a class="pagination-previous has-background-white" href="./patient.php?page2=1">首页</a>
							  <a class="pagination-previous has-background-white" href="./patient.php?page2=<?=$page2-1; ?>"><</a>
							  <ul class="pagination-list">
									<?php
										for($p=1;$p<=$max_page2;$p++):
											if($p==$page2):
									?>
												<li><a class="pagination-link is-current" href="./patient.php?page2=<?=$p?>"><?=$p?></a></li>
									<?php
											else:
									?>
												<li><a class="pagination-link has-background-white" href="./patient.php?page2=<?=$p?>"><?=$p?></a></li>
									<?php
											endif;
										endfor;
									?>
							  </ul>
							  <a class="pagination-next has-background-white" href="./patient.php?page2=<?=$page2+1; ?>">></a>
							  <a class="pagination-next has-background-white" href="./patient.php?page2=<?=$max_page2; ?>">尾页</a>
							</nav>
			</div>
		</div>
