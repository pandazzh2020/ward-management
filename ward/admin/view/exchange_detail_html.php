...
<section class="section">
			...
			<div class="box" data-aos="flip-right" data-aos-duration="800" data-aos-once="true">
				...
				<table style="width: 100%;border-collapse:separate; border-spacing:0px 10px;">
					...
					<tr>
						...
						<td style="padding-left: 15px;">
							<a href="./patient_detail.php?id=<?=$exchange_detail['patient_id']?>">
					...
					<tr>
						...
						<td style="padding-left: 15px;">
							<?php
								if(empty($exchange_detail['from_ward_id'])):
							?>
								<span>暂未安排</span>
							<?php
								else:
							?>
								<a href="./ward_detail?id=<?=$exchange_detail['from_ward_id']?>">
									<?=$exchange_detail['from_ward_building']?>号楼&nbsp;<?=$exchange_detail['from_ward_number']?>户
								</a>
							<?php
								endif;
							?>
					...
					<tr>
						...
						<td style="padding-left: 15px;">
							<a href="./ward_detail?id=<?=$exchange_detail['to_ward_id']?>">
								<?=$exchange_detail['to_ward_building']?>号楼&nbsp;<?=$exchange_detail['to_ward_number']?>户
							</a>
						</td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td style="padding-left: 15px;">
							<strong><?=$exchange_detail['doctor_response']?></strong>
						</td>
					</tr>
					<tr>
						<td colspan="2">
					...
					<tr>
						...
						<td class="has-text-centered">
							<a class="button is-warning is-small" onclick="checkExchange()">一键移至目标病房</a>
						</td>
					</tr>
					<tr>
						<td colspan="2">
	...
	<!-- 提交用的隐藏表单 -->
	<input type="hidden" name="patient_id" value="<?=$exchange_detail['patient_id']?>" />
	<input type="hidden" name="to_ward_id" value="<?=$exchange_detail['to_ward_id']?>" />
	<input type="hidden" name="func" id="func" value="回复" />
</form>
<script>
	function checkExchange(){
		var r = confirm(<?="'确定要将".$exchange_detail['name']."同学移至目标病房吗？'"?>);
		if (r == true) {
			$("#func").val('移至目标病房');
			$("form").submit();
		}
