	...
	<div class="columns">
		...
		<div class="column is-8">
			<div class="box" data-aos="flip-right" data-aos-duration="800" data-aos-once="true">
				<h2 class="has-text-centered subtitle"><i class="fas fa-exchange-alt"></i>&thinsp;换房申请</h2>
					<?php
						if(empty($exchange_list)):
					?>
						<p class="has-text-centered">暂无换房申请</p>
					<?php
						else:
					?>
					...
					?>
								<tr>
									...
									<td>
										<?php
											if(!empty($row['from_ward_id'])):
										?>
												<a href="./ward_detail?id=<?=$row['from_ward_id']?>">
													<?=$row['from_ward_building']?>号楼&nbsp;<?=$row['from_ward_number']?>户
												</a>
										<?php
											else:
										?>
									...
									<td>
										<a href="./ward_detail?id=<?=$row['to_ward_id']?>">
											<?=$row['to_ward_building']?>号楼&nbsp;<?=$row['to_ward_number']?>户
										</a>
									</td>
									<td>
										<?php
